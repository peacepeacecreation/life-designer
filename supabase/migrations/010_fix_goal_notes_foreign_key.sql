-- Fix goal_notes foreign key constraint
-- The table was created with reference to auth.users instead of users table

-- Drop existing table (it's empty anyway)
...
CREATE INDEX idx_goal_notes_user_id ON goal_notes(user_id);
CREATE INDEX idx_goal_notes_created_at ON goal_notes(created_at DESC);
